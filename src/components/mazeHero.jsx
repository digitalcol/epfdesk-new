...
const getGridSize = () => {
  if (typeof window === "undefined") return [16, 10]; // safe fallback for SSR
  if (window.innerWidth <= 480) return [10, 6];
  if (window.innerWidth <= 768) return [12, 8];
  return [16, 10];
};
...
const RawMazeGame = () => {
  // ✅ start with default values (safe for SSR)
  const [[width, height], setGridSize] = useState([16, 10]);
  const [grid, setGrid] = useState([]);
  const [playerPos, setPlayerPos] = useState({ x: 0, y: 0 });
  const [goalPos, setGoalPos] = useState({ x: 15, y: 9 });
  const [keys, setKeys] = useState([]);
  const [powerUps, setPowerUps] = useState([]);
  const [obstacles, setObstacles] = useState([]);
  const [trail, setTrail] = useState([]);
  ...
  // ✅ update grid size on client + window resize
  useEffect(() => {
    const updateSize = () => {
      const size = getGridSize();
      setGridSize(size);
      setGoalPos({ x: size[0] - 1, y: size[1] - 1 });
    };
  ...
  const setupGame = useCallback(() => {
    const newGrid = generateGrid(width, height);
    createPath(newGrid, width - 1, height - 1);
    const avoid = [{ x: 0, y: 0 }, { x: width - 1, y: height - 1 }];
    setGrid(newGrid);
    setPlayerPos({ x: 0, y: 0 });
    setCollectedKeys(0);
    setTrail([]);
  ...
  const movePlayer = useCallback(
    ...
    [playerPos, grid, obstacles, keys, goalPos, gameWon, setupGame]
  );
